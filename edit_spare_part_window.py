...
from settings import today, root_logger, STORE_SPARE_PARTS_ROOT
import sqlalchemy
...
from db import Brother, Canon, Epson, Konica, Kyocera, Lexmark, Oki, Ricoh, Samsung, Sharp, Orders, store_session
...
class Ui_edit_spare_parts_window(QMainWindow):  # Πρέπει να κληρονομήσει απο QMainWindow for pyqtSignal to work
    ...
    def edit_spare_part(self):
        ...
        # Show images
        self.images_path = os.path.join(STORE_SPARE_PARTS_ROOT, f"{self.selected_table.__tablename__}",
                                        f"{self.selected_id}")
